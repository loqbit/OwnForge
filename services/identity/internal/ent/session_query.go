...
type SessionQuery struct {
	...
	withUser       *UserQuery
	withApp        *AppQuery
	withSSOSession *SsoSessionQuery
...
func (_q *SessionQuery) QueryApp() *AppQuery {
...
// QuerySSOSession chains the current query on the "sso_session" edge.
func (_q *SessionQuery) QuerySSOSession() *SsoSessionQuery {
...
func (_q *SessionQuery) QueryIdentity() *UserIdentityQuery {
...
func (_q *SessionQuery) Clone() *SessionQuery {
	...
	return &SessionQuery{
		...
		withSSOSession: _q.withSSOSession.Clone(),
...
func (_q *SessionQuery) WithApp(opts ...func(*AppQuery)) *SessionQuery {
	query := (&AppClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithSSOSession tells the query-builder to eager-load the nodes that are connected to
// the "sso_session" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SessionQuery) WithSSOSession(opts ...func(*SsoSessionQuery)) *SessionQuery {
	query := (&SsoSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSSOSession = query
	return _q
}
...
func (_q *SessionQuery) WithIdentity(opts ...func(*UserIdentityQuery)) *SessionQuery {
	query := (&UserIdentityClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(session.FieldSessionTokenHash).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Session.Query().
//		Select(session.FieldSessionTokenHash).
//		Scan(ctx, &v)
...
func (_q *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	var (
		...
		loadedTypes = [4]bool{
			_q.withUser != nil,
			_q.withApp != nil,
			_q.withSSOSession != nil,
			_q.withIdentity != nil,
		}
	)
	if _q.withUser != nil || _q.withApp != nil || _q.withSSOSession != nil || _q.withIdentity != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withApp; query != nil {
		if err := _q.loadApp(ctx, query, nodes, nil,
			func(n *Session, e *App) { n.Edges.App = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSSOSession; query != nil {
		if err := _q.loadSSOSession(ctx, query, nodes, nil,
			func(n *Session, e *SsoSession) { n.Edges.SSOSession = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withIdentity; query != nil {
		if err := _q.loadIdentity(ctx, query, nodes, nil,
			func(n *Session, e *UserIdentity) { n.Edges.Identity = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SessionQuery) loadApp(ctx context.Context, query *AppQuery, nodes []*Session, init func(*Session), assign func(*Session, *App)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Session)
	for i := range nodes {
		if nodes[i].app_sessions == nil {
			continue
		}
		fk := *nodes[i].app_sessions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "app_sessions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SessionQuery) loadSSOSession(ctx context.Context, query *SsoSessionQuery, nodes []*Session, init func(*Session), assign func(*Session, *SsoSession)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Session)
	for i := range nodes {
		if nodes[i].sso_session_sessions == nil {
			continue
		}
		fk := *nodes[i].sso_session_sessions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(ssosession.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sso_session_sessions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SessionQuery) loadIdentity(ctx context.Context, query *UserIdentityQuery, nodes []*Session, init func(*Session), assign func(*Session, *UserIdentity)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Session)
	for i := range nodes {
		if nodes[i].user_identity_sessions == nil {
			continue
		}
		fk := *nodes[i].user_identity_sessions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_identity_sessions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
