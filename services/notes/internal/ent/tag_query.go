...
func (_q *TagQuery) QuerySnippets() *SnippetQuery {
...
func (_q *TagQuery) WithSnippets(opts ...func(*SnippetQuery)) *TagQuery {
	query := (&SnippetClient{config: _q.config}).Query()
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withSnippets; query != nil {
		if err := _q.loadSnippets(ctx, query, nodes,
			func(n *Tag) { n.Edges.Snippets = []*Snippet{} },
			func(n *Tag, e *Snippet) { n.Edges.Snippets = append(n.Edges.Snippets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadSnippets(ctx context.Context, query *SnippetQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Snippet)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.SnippetsTable)
		s.Join(joinT).On(s.C(snippet.FieldID), joinT.C(tag.SnippetsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.SnippetsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.SnippetsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Snippet](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "snippets" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
